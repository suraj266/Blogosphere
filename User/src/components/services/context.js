...
const categoryList = async () => {
    try {
        const res = await postRequestWithFetch('Category/list')
        if (res.status === true) {
            return res.category
        } else {
            notifyError({ Message: "Oops elswe! Some error Occurs" })
        }
    } catch (err) {
        notifyError({ Message: "Oops catch! Some error Occurs" })
    }
}

const CreateBlog = async (data) => {
    try {
        const res = await postRequestWithFetch("Post/add", data);
        if (res.status) {
...
export { login, handleLogOut, RegisterUser, myProfile, updateProfile, forgotPassword, verifyOtp, changePassword, categoryList, CreateBlog }